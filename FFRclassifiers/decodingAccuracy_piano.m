...
% arrange results
    ...
    %for every column that contains the value 1500 (or 2000) in 'totsizes', we want
...
%finds all of the columns where the element in totsizes is 1500
%[~,cols1500] = find(totsizes == 1500);

%finds all of the columns where the element in totsizes is 2000
%[~,cols2000] = find(totsizes == 2000)

%takes every 7th element starting at the 3rd element of cpassive/cactive and stores it in a new array
%every 7th element corresponds to the column where there is a 1500 in totsizes 
corrpass1500 = cpassive(3:7:end); 
corract1500 = cactive(3:7:end);
crates1500_piano = crates(3:7:end);

%takes every 7th element starting at the 4th element of cpassive/cactive and stores it in a new array
...
crates2000_piano = crates(4:7:end);


%T-test for sample size 1500
[Hypothesis,PValue,ConfidenceInterval, STATS] = ttest(corrpass1500,corract1500)

%T-test for sample size 2000
[Hypothesis,PValue,ConfidenceInterval, STATS] = ttest(corrpass2000,corract2000)
...
%     title('HMM Decoding Accuracy for the Piano Tone'); 
%     
